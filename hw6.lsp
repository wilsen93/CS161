...
(defun at-least-one-color (n c k)
  (cond ((or (< k 1) (< k c)) nil)
	((> k c) (append (list (node2var n c k)) (at-least-one-color n (+ 1 c) k)))
	(t (list (node2var n c k)))
	)
  )
...
(defun at-most-one-color (n c k)
  (cond ((= c k) (list (list (* n k) (* (* n k) -1))))
	(t (helper1 (at-least-one-color n c k)))
	)
  )

(defun helper1 (clause)
  (cond ((<= (length clause) 1) nil)
	(t (append (helper2 (* (first clause) -1) (rest clause)) (helper1 (rest clause))))
	)
  )

(defun helper2 (f s)
  (cond ((null s) nil)
	(t (append (list (list f (* (- 0 1) (first s)))) (helper2 f (rest s))))
	)
  )
; EXERCISE: Fill this function
...
(defun helper3 (e c k)
  (cond ((< k c) nil)
	(t (append (list (list (* (node2var (first e) c k) -1) (* (node2var (second e) c k) -1))) (helper3 e (+ 1 c) k)))
	)
  )
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
